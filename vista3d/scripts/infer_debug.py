...
class InferClass:
    def __init__(self, config_file="./configs/infer.yaml", **override):
        ...
        # _args = {'infer': 
        #             {'ckpt_name': "$@bundle_root + '/test_model.pt'", 'output_path': "$@bundle_root + '/prediction'", 'log_output_file': "$@bundle_root + '/inference.log'"}
        #         }
        parser = ConfigParser()
...
if __name__ == "__main__":
    # # orig usage: python -m scripts.infer --config_file 'configs/infer.yaml' - infer --image_file 'example-1.nii.gz' --label_prompt "[1]" --save_mask true
    # fire, _ = optional_import("fire")
    # fire.Fire(InferClass)
    """
    Debugging and testing script for InferClass.
    This script directly sets the parameters and runs inference without using command-line arguments.
    """

    # Define parameters for debugging
    # config parameters
    config_file = "configs/infer.yaml"  # Path to the configuration file
    ckpt_name = "models/model.pt"  # Path to the checkpoint file
    output_path = "models/debug_prediction"  # Path to save the output
    
    override = {
        "infer": {
            "ckpt_name": ckpt_name,
            "output_path": output_path,
            "log_output_file": "models/debug_inference.log",
        }
    }
    # Infer parameters
    image_file = "example-1.nii.gz"  # Path to the input image file
    label_prompt = [1]  # Label prompt for inference
    save_mask = True

    # Create an instance of InferClass
    infer_instance = InferClass(config_file=config_file, **override)

    # Perform inference
    result = infer_instance.infer(
